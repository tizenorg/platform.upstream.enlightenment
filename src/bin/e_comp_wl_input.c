...
e_coml_wl_input_keymap_path_get(const char *rules, const char *model, const char *layout)
{
...
e_comp_wl_input_keymap_compile(struct xkb_context *ctx, const char *rules, const char *model, const char *layout, char **keymap_path)
{
   ...
   if (e_config->xkb.use_cache)
     {
        cache_path = (char *)e_coml_wl_input_keymap_path_get(rules, model, layout);
                           ...
                           struct xkb_context *dflt_ctx, struct xkb_keymap *dflt_map)
{
   ...
   if (use_dflt_xkb)
     {
        keymap = dflt_map;
        keymap_path = (char *)e_coml_wl_input_keymap_path_get(rules, model, layout);
     }
   else
     keymap = e_comp_wl_input_keymap_compile(cdata->xkb.context, rules, model, layout, &keymap_path);
