...
_e_comp_wl_input_keymap_cache_create(char *keymap_path, char *keymap_data)
{
   ...
   if (EINA_FALSE == e_config->xkb.cache_enable) return;
...
e_comp_wl_input_keymap_set(E_Comp_Data *cdata, const char *rules, const char *model, const char *layout)
{
   ...
   if (e_config->xkb.cache_enable)
     {
        keymap_path = E_NEW(char, 128);
        snprintf(keymap_path, 128, "/var/lib/xkb/%s-%s-%s-%s-%s.xkb",
   ...
   if (!file)
     ...
     {
        INF("Find a keymap file(%s). Generate keymap using file\n", keymap_path);
   ...
   /* cleanup */
   if (keymap_path) E_FREE(keymap_path);
