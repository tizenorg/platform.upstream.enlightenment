...
static void
_e_comp_wl_device_last_device_set(Ecore_Device_Class dev_class, void *device)
{
   ERR("_e_comp_wl_device_last_device_set() dev_class: %d, ", dev_class);
   switch (dev_class)
     {
      case ECORE_DEVICE_CLASS_MOUSE:
         e_comp_wl->input_device_manager.last_device_ptr = (Evas_Device *)device;
         break;
      case ECORE_DEVICE_CLASS_KEYBOARD:
         e_comp_wl->input_device_manager.last_device_kbd = (Ecore_Device *)device;
         break;
      case ECORE_DEVICE_CLASS_TOUCH:
         e_comp_wl->input_device_manager.last_device_touch = (Evas_Device *)device;
         break;
      default:
         break;
     }
...
static void
_e_comp_wl_device_client_last_device_set(E_Client *ec, Ecore_Device_Class dev_class, void *device)
{
   ERR("_e_comp_wl_device_client_last_device_set() dev_class: %d, ", dev_class);
   switch (dev_class)
     {
      case ECORE_DEVICE_CLASS_MOUSE:
         ec->comp_data->last_device_ptr = (Evas_Device *)device;
         break;
      case ECORE_DEVICE_CLASS_KEYBOARD:
         ec->comp_data->last_device_kbd = (Ecore_Device *)device;
         break;
      case ECORE_DEVICE_CLASS_TOUCH:
         ec->comp_data->last_device_touch = (Evas_Device *)device;
         break;
      default:
         break;
     }
...
{
   Evas_Device *last_device, *ec_last_device;
   ...
   last_device = (Evas_Device *)_e_comp_wl_device_last_device_get(dev_class);
   ec_last_device = (Evas_Device *)_e_comp_wl_device_client_last_device_get(ec, dev_class);
   if (!last_device || (last_device != dev) || !ec_last_device || (last_device != ec_last_device))
     {
        _e_comp_wl_device_last_device_set(dev_class, dev);
        _e_comp_wl_device_client_last_device_set(ec, dev_class, dev);
...
_e_comp_wl_device_send_last_event_device(E_Client *ec, Ecore_Device_Class dev_class, uint32_t timestamp)
{
   ...
   if (dev_class == ECORE_DEVICE_CLASS_KEYBOARD)
     dev_name = ecore_device_identifier_get((Ecore_Device *)last_device);
   else
     dev_name = evas_device_description_get((Evas_Device *)last_device);
...
_e_comp_wl_evas_cb_focus_in_timer(E_Client *ec)
{
   ...
   EINA_LIST_FOREACH(e_comp_wl->kbd.focused, l, res)
     {
        if (e_comp_wl->kbd.keys.size > 0)
          {
             wc = wl_resource_get_client(ec->comp_data->surface);
             if (wl_resource_get_client(res) == wc)
               _e_comp_wl_device_send_last_event_device(ec, ECORE_DEVICE_CLASS_KEYBOARD, t);
          }
...
_e_comp_wl_send_event_device(struct wl_client *wc, uint32_t timestamp, Ecore_Device *dev, uint32_t serial)
{
   ...
   last_device = (Ecore_Device *)_e_comp_wl_device_last_device_get(ECORE_DEVICE_CLASS_KEYBOARD);
   if (!last_device || (last_device != dev))
     {
        _e_comp_wl_device_last_device_set(ECORE_DEVICE_CLASS_KEYBOARD, dev);
