...
static Eldbus_Message* _e_test_helper_cb_get_target_window(const Eldbus_Service_Interface *iface, const Eldbus_Message *msg);
...
static const Eldbus_Method methods[] ={
     ...
     },
     {
        "GetRegistrant",
        NULL,
        ELDBUS_ARGS({"u", "registered window id"}),
        _e_test_helper_cb_get_target_window, 0
     },
     {
        "GetClients",
        NULL,
                                    ...
                                    const Eldbus_Message *msg)
{
   ...
   eldbus_message_arguments_append(reply, "b", (th_data->target.win == id) && (th_data->target.vis != 1));
   ...
   if (th_data->target.vis < 1) _e_test_helper_target_clear();

   return reply;
}

static Eldbus_Message *
_e_test_helper_cb_get_target_window(const Eldbus_Service_Interface *iface EINA_UNUSED,
                                    const Eldbus_Message *msg)
{
   Eldbus_Message *reply = eldbus_message_method_return_new(msg);
   ...
   eldbus_message_arguments_append(reply, "u", th_data->target.win);
                                    ...
                                    void *event)
{
   ...
   if ((th_data->target.disuse) && (!th_data->target.vis))
...
_e_test_helper_cb_client_remove(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (!th_data->target.ec) return ECORE_CALLBACK_PASS_ON;
   if (ec != th_data->target.ec) return ECORE_CALLBACK_PASS_ON;
