...
/* global variables */
static Eina_List *slot_list = NULL;
...
static Eina_Bool _e_slot_check_changes(E_Slot *slot);
...
e_slot_update (int slot_id)
{
   ...
   slot = _e_slot_find_by_id(slot_id);
   if (!slot) return;
   if (!_e_slot_check_changes(slot)) return;
...
static Eina_Bool
_e_slot_check_changes(E_Slot *slot)
{
   if (!slot) return EINA_FALSE;
   if (slot->changed)
     {
        ELOGF("SLOT", "need to update - changes.. true", 0,0);
        slot->changed = EINA_FALSE;
        return EINA_TRUE;
     }
        ...
        EINA_LIST_FOREACH(slot->client_list, l, slot_client)
          {
             ...
             if (x != slot->rect.x ||
                 ...
                 h != slot->rect.h)
               {
                  ELOGF("SLOT", "need to update - (cur: %d,%d,%dx%d->slot: %d,%d,%dx%d)", slot_client->ec->pixmap, slot_client->ec, x,y,w,h, slot->rect.x, slot->rect.y, slot->rect.w, slot->rect.h);
                  changes = EINA_TRUE;
                  break;
               }
          }
