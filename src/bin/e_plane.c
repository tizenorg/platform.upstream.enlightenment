                       ...
                       E_Client *ec)
{
   ...
   eina_stringshare_replace(_e_plane_ec_last_err, "Type dismatched : ec not availabe on e_plane");
err:
...
{
   EINA_SAFETY_ON_NULL_RETURN_VAL(plane, EINA_FALSE);
   if (plane->type == E_PLANE_TYPE_PRIMARY) return EINA_TRUE;
   return EINA_FALSE;
}
