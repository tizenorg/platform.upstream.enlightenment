...
EINTERN E_Plane *
e_plane_new(E_Output_Screen *screen, int zpos)
{
   E_Plane *plane;
   ...
   plane->geometry.x = screen->config.geom.x;
   plane->geometry.y = screen->config.geom.y;
   plane->geometry.w = screen->config.geom.w;
   plane->geometry.h = screen->config.geom.h;
