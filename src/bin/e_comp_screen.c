...
_e_comp_screen_keymap_set(struct xkb_context **ctx, struct xkb_keymap **map)
{
   ...
   if (default_rules) names.rules = strdup(default_rules);
   if (default_model) names.model = strdup(default_model);
   if (default_layout) names.layout = strdup(default_layout);
