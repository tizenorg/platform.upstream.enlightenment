   ...
   for (o = evas_object_top_get(zone->comp->evas); o; o = evas_object_below_get(o))
     {
        ...
        EINA_ITERATOR_FOREACH(itr, _r)
          {
             if (E_INTERSECTS(ec->x, ec->y, ec->w, ec->h, _r->x, _r->y, _r->w, _r->h))
               {
        ...
        if ((e_comp_object_is_hidden(ec->frame)) || (!evas_object_visible_get(ec->frame)) ||
            (e_object_is_del(E_OBJECT(ec))) || (!ec->visible))
