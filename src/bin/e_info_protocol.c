...
static BINARY_TREE_NODE _parser_token_parse(BINARY_TREE tree, TOKEN_DATA token);
...
TOKEN
_parser_next_token_get(const char ** string)
{
...
static void
_parser_token_process(TOKEN_DATA token)
{
   do
     {
        token->last_symbol = *(token->string);
        token->last_token = _parser_next_token_get(token->string);
...
_parser_statement_parse(BINARY_TREE tree, TOKEN_DATA token)
{
   ...
   if (token->last_token == BET_L_BR)
     {
        _parser_token_process(token);

        node = _parser_token_parse(tree, token);
        if (!node)
          return NULL;
...
static BINARY_TREE_NODE
_parser_token_parse(BINARY_TREE tree, TOKEN_DATA token)
{
   BINARY_TREE_NODE node, left = NULL, right = NULL;
   PARSE_DATA data;

   node = _parser_statement_parse(tree, token);
   if (!node)
     {
   ...
   while (token->last_token == BET_AND)
     {
        ...
        right = _parser_statement_parse(tree, token);
        if (!right)
          goto fail;
   ...
   if (token->last_token == BET_OR)
     {
        ...
        right = _parser_token_parse(tree, token);
        if (!right)
          goto fail;
...
BINARY_TREE
_parser_rule_string_parse(const char * string)
{
   BINARY_TREE tree;
   ...
   tree = bintree_create_tree(sizeof(struct _PARSE_DATA));
   node = _parser_token_parse(tree, &token);
   if (!node)
     {
...
static int
_rulechecker_string_compare(COMPARER comparer, char * str2, char * str1)
{
...
static int
_rulechecker_int_compare(COMPARER comparer, int int2, int int1)
{
   switch (comparer)
     {
        case EQUAL:
...
static int
_rule_print_func(BINARY_TREE tree, BINARY_TREE_NODE node, BINARY_TREE_NODE parent, void * arg)
{
...
static int
_rule_validate_func(BINARY_TREE tree, BINARY_TREE_NODE node, BINARY_TREE_NODE parent, void * arg)
{
   VAL_ARGUMENTS * args = (VAL_ARGUMENTS*)arg;
   BINARY_TREE_NODE left, right;
   PARSE_DATA data, left_data= NULL, right_data = NULL;
        ...
        if (!strcasecmp(data->variable_name, "TYPE"))
          {
             ...
             if (_rulechecker_string_compare(data->comparer, data->value.string, type_string))
               data->result = BEP_TRUE;
             else
          ...
          {
             if (msg && _rulechecker_string_compare(data->comparer, data->value.string, iface))
               data->result = BEP_TRUE;
             else
               data->result = BEP_FALSE;
          }
          ...
          {
             if (msg && _rulechecker_string_compare(data->comparer, data->value.string, msg))
               data->result = BEP_TRUE;
             else
               data->result = BEP_FALSE;
          }
          ...
          {
             if (_rulechecker_int_compare(data->comparer, data->value.integer, args->pid))
               data->result = BEP_TRUE;
             else
               data->result = BEP_FALSE;
          }
          ...
          {
             if (args->cmd && _rulechecker_string_compare(data->comparer, data->value.string, args->cmd))
               data->result = BEP_TRUE;
             else
               data->result = BEP_FALSE;
          }
...
RC_RESULT_TYPE
_rulechecker_rule_add(RULE_CHECKER rc, POLICY_TYPE policy, const char * rule_string)
{
   if (rc->count == MAX_RULE)
     return RC_ERR_TOO_MANY_RULES;

   rc->rules[rc->count].tree = _parser_rule_string_parse(rule_string);
...
RC_RESULT_TYPE
_rulechecker_rule_remove(RULE_CHECKER rc, int index)
{
...
void
_rulechecker_rule_print(RULE_CHECKER rc, char *reply, int *len)
{
   ...
   for (i = 0; i < rc->count; i++)
     {
        ...
        bintree_inorder_traverse(rc->rules[i].tree, _rule_print_func, (void*)&buffer);
        REPLY("\"\n");
     }
}

const char *
_rulechecker_usage_print()
{
   return
        "##########################################################\n"
...
void
_rulechecker_destroy(RULE_CHECKER rc)
{
   int i;
...
int
_rulechecker_rule_validate(RULE_CHECKER rc, int type, int reqID, const char * name, int pid, char * cmd)
{
   ...
   for (i = rc->count - 1; i >= 0; i--)
     {
        bintree_postorder_traverse(rc->rules[i].tree, _rule_validate_func, &args);
   ...
   if (!rc)
     {
        ...
        if (!rc)
          {
             REPLY ("failed: create rulechecker\n");
             return EINA_FALSE;
          }
     }
