...
static Eina_Bool
_e_comp_hwc_output_create(E_Comp_Hwc *hwc, int index)
{
   E_Comp_Hwc_Output *hwc_output = NULL;
   E_Comp_Hwc_Renderer *hwc_renderer = NULL;
   E_Comp_Hwc_Layer *hwc_layer = NULL;
   ...
   toutput = tdm_display_get_output(hwc->tdisplay, index, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(hwc_output, EINA_FALSE);
   ...
   for (i = 0; i < num_layers; i++)
     {
        ...
        EINA_SAFETY_ON_NULL_RETURN_VAL(hwc_renderer, EINA_FALSE);
...
e_comp_hwc_init(void)
{
   ...
   for (i = 0; i < num_outputs; i++)
     {
        if (_e_comp_hwc_output_create(hwc, i) != EINA_TRUE)
          goto fail;
     }
#else
   /* make only main output list for hwc */
      hwc->num_outputs = 1;
   if (_e_comp_hwc_output_create(hwc, 0) != EINA_TRUE)
     goto fail;
#endif
