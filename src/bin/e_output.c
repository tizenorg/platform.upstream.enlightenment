...
static void
_e_output_canvas_render_post(void *data EINA_UNUSED, Evas *e EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   tdm_output_get_conn_status(output->toutput, &status);
   if (status == TDM_OUTPUT_CONN_STATUS_DISCONNECTED)
     {
        WRN("tdm_output disconnected");
        return;
     }
...
#if 0
   evas_event_callback_add(e_comp->evas, EVAS_CALLBACK_RENDER_POST, _e_output_canvas_render_post, output);
#endif
...
EINTERN Eina_Bool
e_output_apply(E_Output *output, E_Output_Mode *mode)
{
   tdm_error error;
