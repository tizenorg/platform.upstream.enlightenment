...
_hwc_prepare_set(E_Output * eout, int n_vis, Eina_List* clist)
{
   ...
   if ((n_ec == n_vis) &&
     ...
     {
        int del = n_ly - n_ec;
        INF("HWC : \t fully hwc \t n_ec(%d), nVis(%d), n_ly(%d), HWMode(%d) \n", n_ec, n_vis, n_ly, e_comp->hwc_mode);
   ...
   else if ((n_ly < n_vis) || // hybrid
            (n_ec < n_vis))
     {
        int del = n_ly - n_ec;
        INF("HWC : \t hybrid \t n_ec(%d), nVis(%d), n_ly(%d), HWMode(%d) \n", n_ec, n_vis, n_ly, e_comp->hwc_mode);
        EINA_LIST_REVERSE_FOREACH(hwc_l, l, ep)
          {
             ec = NULL;
             if (del > 0){ ep->prepare_ec = NULL; del--; continue;}
...
_e_comp_cb_idle(void)
{
   ...
   if (!e_comp->hwc) return ECORE_CALLBACK_RENEW;
   if (!e_comp->hwc_need_update) return ECORE_CALLBACK_RENEW;
   if (!_e_comp_hwc_is_on()) return ECORE_CALLBACK_RENEW;
                  ...
                  if (e_plane_is_primary(ep))
                    {
                       ...
                       if (ep->ec != NULL)
                         {
                            buffer = e_pixmap_resource_get(ep->ec->pixmap);
                            if (buffer) e_comp_hwc_display_client(ep->ec);
                         }

                       e_comp->hwc_need_update = EINA_FALSE;
                       return ECORE_CALLBACK_RENEW;
                    }
                  continue;
               }
