   ...
   if (dim_client != ec)
     {
        ...
        if (dim_client && e_comp_objs_dim_enable_get(dim_client))
           prev_dim = EINA_TRUE;

        if (prev_dim && dim_client->visible && ec)
          {
             e_comp_objs_dim_enable_set(dim_client, EINA_FALSE, EINA_TRUE);
             e_comp_objs_dim_enable_set(ec, EINA_TRUE, EINA_TRUE);
          }
        else
          {
             if (prev_dim) e_comp_objs_dim_enable_set(dim_client, EINA_FALSE, EINA_FALSE);
